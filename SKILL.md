---
name: xiaowukong-illustrations
description: 生成小悟空风格的中文正文配图。用于用户要求为中文文章、帖子、博客、Notion 文档、工作流文档、方法论、流程、结构、状态、隐喻或观点生成"小悟空""龙珠配图""悟空插图""手绘配图""正文配图""文章插图""配图建议""shot list""去标题/改图"等任务；默认使用小悟空 IP（七龙珠幼年悟空形象）、纯白手绘底色、小悟空配色、少量红橙蓝中文批注、简洁清爽但热血天马行空的视觉风格。
---
...
为中文文章设计和生成 16:9 横版正文配图。目标不是做商业插画、PPT 信息图或可爱卡通，而是把文章里的关键判断、流程、结构、状态或隐喻，变成一张清爽、热血、有创意、可读但不说明书的手绘解释图。

默认视觉 IP 是"小悟空"：七龙珠幼年孙悟空形象——黑色刺猬头、白色无袖背心、蓝色灯笼裤、红色护腕、肩挎如意棒。表情天真热血但认真。小悟空必须参与画面的核心动作，不能只是站在旁边当装饰。

> **参考形象**：`assets/xiaowukong-three-view.png` 是小悟空的标准三视图（正面/侧面/背面），生成时以此为角色基准。
...
- `references/xiaowukong-ip.md`：小悟空 IP 的形象、性格、动作库和禁忌。
...
- 纯白背景
- 手绘线稿风格
- 小悟空作为核心动作主体（黑刺猬头、白背心、蓝灯笼裤、红护腕、如意棒）
- 少量红色/橙色/蓝色中文手写批注
- 大量留白
- 禁止 PPT、商业插画、幼稚可爱、复杂架构、左上角类型标题

不要复刻过往案例。每次都要从当前文章重新发明一个奇怪但成立、热血但不幼稚的隐喻。
